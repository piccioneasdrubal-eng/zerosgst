...
(() => {
  ...
  function boot() {
    ...
    function startMusic() {
      ...
      if (!musicAudio) {
        musicAudio = new Audio('assets/brutal-dubstep-drop.wav');
      ...
      if (playPromise && typeof playPromise.catch === 'function') playPromise.catch(() => { musicPlaying = false; });
    }
    function stopMusic() {
      musicPlaying = false;
    ...
    function renderCellPosition(c, dt = 1/60) {
      ...
      const tangent = Math.sin(phase);
      const normalX = -tangent, normalY = Math.cos(phase);
      let r = renderCells.get(c.id);
      if (!r) {
        const len = Math.hypot(svx, svy);
        const nx = len > 0.001 ? svx / len : 0, ny = len > 0.001 ? svy / len : 0;
        const back = inSplitLaunch ? Math.min(85, len * 0.075) : 0;
        r = { x:c.x - nx * back, y:c.y - ny * back, spawnAt:now };
        renderCells.set(c.id, r);
      }
      const posK = 1 - Math.exp(-(inSplitLaunch ? 11 : 15) * dt);
      let tx = c.x, ty = c.y;
      if (inSplitLaunch) {
        // Agar.io-like arc: strong launch early, then a smooth curved return.
