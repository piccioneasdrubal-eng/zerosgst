...
function token_user_e(string $token): ?array { $p=explode('.',trim($token)); if(count($p)!==3||$p[0]!=='zl1'||!defined('API_SECRET')||API_SECRET==='')return null; $sig=rtrim(strtr(base64_encode((string)hash_hmac('sha256',$p[1],(string)API_SECRET,true)),'+/','-_'),'='); if(!hash_equals($sig,$p[2]))return null; $b=$p[1]; $b.=str_repeat('=',(4-strlen($b)%4)%4); $x=json_decode((string)base64_decode(strtr($b,'-_','+/'),true),true); if(!is_array($x)||(int)($x['exp']??0)<time())return null; $st=db()->prepare('SELECT * FROM zl_users WHERE id=? LIMIT 1'); $st->execute([(int)($x['uid']??0)]); return $st->fetch()?:null; }
...
function user_wallet_e(int $uid): array { $c=(int)db()->query('SELECT coins FROM zl_users WHERE id='.(int)$uid)->fetchColumn(); $st=db()->prepare('SELECT item_id,qty,equipped FROM zl_inventory WHERE user_id=? AND qty>0 ORDER BY item_id');$st->execute([$uid]);$items=[];$eq='default';while($r=$st->fetch()){for($i=0;$i<(int)$r['qty']&&count($items)<100;$i++)$items[]=$r['item_id'];if((int)$r['equipped']===1)$eq=$r['item_id'];}return ['coins'=>$c,'inventory'=>$items,'equippedSkin'=>$eq]; }
...
if($action==='buy'){
 ...
 try{
  $s=db()->prepare('SELECT coins FROM zl_users WHERE id=? FOR UPDATE');$s->execute([$uid]);$coins=(int)$s->fetchColumn();
  ...
  db()->prepare('UPDATE zl_users SET coins=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')->execute([$new,$uid]);
...
if($action==='equip'){
 $item=(string)($b['item_id']??''); $st=db()->prepare('SELECT qty FROM zl_inventory WHERE user_id=? AND item_id=? AND qty>0');$st->execute([$uid,$item]); if(!$st->fetch())oute(['ok'=>false,'error'=>'Skin non posseduta.'],409); db()->prepare('UPDATE zl_inventory SET equipped=0 WHERE user_id=?')->execute([$uid]); db()->prepare('UPDATE zl_inventory SET equipped=1 WHERE user_id=? AND item_id=?')->execute([$uid,$item]); $skin=preg_replace('/^skin_/','',$item); db()->prepare('UPDATE zl_users SET equipped_skin=?,updated_at=CURRENT_TIMESTAMP WHERE id=?')->execute([$skin,$uid]);oute(['ok'=>true,'wallet'=>user_wallet_e($uid)]);
}
if($action==='unequip'){db()->prepare('UPDATE zl_inventory SET equipped=0 WHERE user_id=?')->execute([$uid]);db()->prepare("UPDATE zl_users SET equipped_skin='default',updated_at=CURRENT_TIMESTAMP WHERE id=?")->execute([$uid]);oute(['ok'=>true,'wallet'=>user_wallet_e($uid)]);}
