...
function ensure_schema(): void {
  ...
  $ut=users_table();
  ensure_column($ut, 'coins', "INT NOT NULL DEFAULT 0");
  ensure_column($ut, 'equipped_skin', "VARCHAR(80) NOT NULL DEFAULT 'default'");
}
function user_row(int $id): array { $ut=users_table(); $s=db()->prepare("SELECT * FROM `$ut` WHERE id=? LIMIT 1"); $s->execute([$id]); $u=$s->fetch(); if(!$u) out(['ok'=>false,'error'=>'Utente non trovato.'],404); return $u; }
...
if($action==='equip' || $action==='unequip') {
  ensure_schema(); $ut=users_table();
  if($action==='unequip') {
    $q=db()->prepare("UPDATE `$ut` SET equipped_skin='default', updated_at=CURRENT_TIMESTAMP WHERE id=?"); $q->execute([$uid]);
  ...
  $u=db()->prepare("UPDATE `$ut` SET equipped_skin=?, updated_at=CURRENT_TIMESTAMP WHERE id=?"); $u->execute([$item,$uid]);
...
if($action==='purchase_item'){
  ...
  try{
    ...
    $c=db()->prepare("SELECT coins FROM `$ut` WHERE id=? FOR UPDATE"); $c->execute([$uid]); $coins=(int)$c->fetchColumn();
    if($coins<$price){ db()->rollBack(); out(['ok'=>false,'error'=>'ZeroCoins insufficienti.','wallet'=>['coins'=>$coins]],409); }
    $new=$coins-$price; $up=db()->prepare("UPDATE `$ut` SET coins=?,updated_at=CURRENT_TIMESTAMP WHERE id=?"); $up->execute([$new,$uid]);
    ...
    out(['ok'=>true,'item'=>$catalog[$item],'wallet'=>wallet($uid)+['price_paid'=>$price]]);
