...
function user(string $t):?array{if(!defined('API_SECRET')||!API_SECRET)return null;$p=explode('.',trim($t));if(count($p)!==3||$p[0]!=='zl1')return null;if(!hash_equals(b64u(hash_hmac('sha256',$p[1],API_SECRET,true)),$p[2]))return null;$j=b64d($p[1]);$x=json_decode((string)$j,true);if(!is_array($x)||(int)($x['exp']??0)<time())return null;$q=db()->prepare('SELECT * FROM zl_users WHERE id=? LIMIT 1');$q->execute([(int)($x['uid']??0)]);return$q->fetch()?:null;}
