(() => {
  ...
  async function customUpload(){const i=$('zl-menu-skin-file');const title=$('zl-menu-skin-title')?.value?.trim();if(!i?.files?.[0]){toast('❌ Seleziona un’immagine.');return;} try{await uploadCustomSkin(i.files[0],title||i.files[0].name.replace(/\.[^.]+$/,''));beep(980,70);}catch(e){toast('❌ '+(e?.message||'Upload fallito'));}}
  async function uploadCustomSkinFromUrl(url, title = 'Skin personalizzata') {
    const token = window.ZLAuth?.getToken?.(); if (!token) throw new Error('Accedi per caricare una skin.');
    if (!/^https?:\/\//i.test(url||'')) throw new Error('Incolla un link diretto all\'immagine (deve iniziare con http:// o https://).');
    const status=$('zl-menu-status'); if(status) status.textContent='⏳ Scarico l\'immagine dal link...';
    const r=await fetch('/auth/skins.php',{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({action:'from_url',token,url,title:(title||'Skin personalizzata').slice(0,64)}),cache:'no-store'});
    const raw=await r.text(); let d={}; try{d=JSON.parse(raw)}catch(_){throw new Error(`Upload da link HTTP ${r.status}`)}; if(!r.ok||!d.ok) throw new Error(d.error||`Upload da link fallito (HTTP ${r.status})`);
    if(status) status.textContent=`✅ Skin caricata: ${d.skin?.title||title}`; setTimeout(()=>loadMySkins?.(),0); return d.skin;
  }
  async function customUploadFromUrl(){const i=$('zl-menu-skin-url');const title=$('zl-menu-skin-title')?.value?.trim();const url=i?.value?.trim();if(!url){toast('❌ Incolla un link immagine.');return;} try{await uploadCustomSkinFromUrl(url,title||'Skin personalizzata');beep(980,70);}catch(e){toast('❌ '+(e?.message||'Upload da link fallito'));}}  function assignKey(action){const btn=document.querySelector(`[data-key-action="${action}"]`); if(!btn)return;btn.textContent='Premi un tasto…';const handler=e=>{e.preventDefault();const key=e.code==='Space'?' ':String(e.key||'').toLowerCase();if(!key)return;let cfg={};try{cfg=JSON.parse(get('agarServerConfig','{}'))||{};}catch(_){}cfg.keybinds=cfg.keybinds||{split:' ',feed:'w',virus:'q',godmode:'g'};cfg.keybinds[action]=key;set('agarServerConfig',JSON.stringify(cfg));btn.textContent=key===' ' ? 'Space' : e.key;window.dispatchEvent(new Event('keybinds-changed'));saveSettings();window.removeEventListener('keydown',handler,true);beep(780,40);};window.addEventListener('keydown',handler,true);}
  function action(id){
    const map={
